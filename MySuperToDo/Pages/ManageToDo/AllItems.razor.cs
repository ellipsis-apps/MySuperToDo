...
/// <summary>
/// Represents the main index page component for managing ToDo lists and items.
/// This component displays a hierarchical tree of ToDo lists and items, supports drag-and-drop for reorganization,
...
public partial class AllItems : IAsyncDisposable
{
    ...
    /// Shows only the "All To Do Items" list and its contents.
    /// </summary>
    private void RebuildTree()
    {
        var allItemsList = _listsBySoul.Values.FirstOrDefault(l => string.Equals(l.Id, AllItemsListId, StringComparison.OrdinalIgnoreCase));
        if (allItemsList is null)
        {
            _treeData = [];
            return;
        }
        var root = BuildListNode(allItemsList, inAllItemsBranch: true);
        root.Expanded = true;
        _treeData = [root];
    }
    ...
    private void OnTreeItemContextMenu(TreeItemContextMenuEventArgs args)
    {
        ...
        var isRootNode = !_contextNode.IsTodoItem && string.Equals(_contextNode.Id, AllItemsListId, StringComparison.OrdinalIgnoreCase);
        if (!isRootNode)
        {
            return;
        }
        ...
        [
            new ContextMenuItem { Text = "Add a To Do item", Value = "todo", Icon = "add" }
        ];
